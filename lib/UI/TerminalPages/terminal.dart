...
class Terminal extends StatefulWidget {
  final String terminal; // Terminal Id parameter
  final int merchantId;

  const Terminal({super.key, required this.terminal, required this.merchantId});
...
class _OrdersPageState extends State<Terminal> {
  ...
  Future<void> _setUpInventory() async {
    try {
      ...
      await inv.fetchMerchantDetails(
          widget.merchantId); // Call fetchMerchantDetails with the merchantId
  ...
  void _updateLists() {
    ...
    setState(() {
      ...
      List<TerminalOrder> claimedOrders = allOrders
          .where((order) =>
              order.terminal == widget.terminal &&
      ...
      if (disabledTerminal &&
          !allOrders.any((order) => order.terminal == widget.terminal)) {
        socket!.sink.add(
          json.encode({
            'action': 'disable',
  ...
  void _executeFunctionForUnclaimed(TerminalOrder order) {
    ...
    final claimRequest = {
      'action': 'claim',
      'terminal': widget.terminal.toString(),
  ...
  void _onOrderTap(TerminalOrder order) {
    if (order.terminal.isEmpty) {
      _executeFunctionForUnclaimed(order);
    } else if (order.terminal == widget.terminal) {
  ...
  Color _getOrderTintColor(TerminalOrder order) {
    ...
    if (order.terminal != '' && order.terminal != widget.terminal) {
      ...
      builder: (BuildContext context) {
        ...
        return StatefulBuilder(
          builder: (BuildContext context, StateSetter setState) {
            return AlertDialog(
              title: Text(
                'Claim tips for ${widget.terminal} (\$${tipAmount.toStringAsFixed(2)})',
          ...
          switch (response.keys.first) {
            ...
            case 'orders':
              ...
              for (TerminalOrder incomingOrder in incomingOrders) {
                ...
                if (incomingOrder.terminal.isEmpty &&
