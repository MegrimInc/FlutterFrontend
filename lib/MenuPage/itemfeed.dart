...
class ItemFeed extends StatefulWidget {
  ...
  @override
  ItemFeedState createState() => ItemFeedState();
}
...
class ItemFeedState extends State<ItemFeed>
  ...
  void _submitOrder(BuildContext context, {required bool inAppPayments}) async {
    ...
    final availablePoints =
        localDatabase.getPointsForMerchant(widget.merchantId)?.points ?? 0;
...
// Private method to save the payment method to the database
                ...
                child: SingleChildScrollView(
                  child: Column(
                    ...
                    children: cart.merchantCart.keys.map((itemId) {
                      return Column(
                        crossAxisAlignment: CrossAxisAlignment.center,
                        children: cart.merchantCart[itemId]!.entries.map((entry) {
  ...
  Widget _buildButtonGroup(
    ...
    final item = LocalDatabase().getItemById(itemId);
    final price =
        (isHappyHour ? item.discountPrice : item.regularPrice) * 1.20;
